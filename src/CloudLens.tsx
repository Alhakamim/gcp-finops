...
function InvoicesPage(){const{t,T}=useApp();const downloadCSV=(inv)=>{const csv=`Invoice ID,Period,Amount,Status\n${inv.id},${inv.period},${inv.amount},${inv.status}`;const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${inv.id}.csv`;a.click();URL.revokeObjectURL(a.href);};const downloadAll=()=>{let csv='Invoice ID,Period,Amount,Status\n';INVOICES_DATA.forEach(inv=>{csv+=`${inv.id},${inv.period},${inv.amount},${inv.status}\n`;});const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='invoices.csv';a.click();URL.revokeObjectURL(a.href);};return(<div style={{display:"flex",flexDirection:"column",gap:24}}><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}><KPICard label={t.kpi.outstanding} value="$0.00" sub={t.kpi.outstandingSub} accent="#34D399"/><KPICard label={t.kpi.lastInvoice} value="$94,100" sub={t.kpi.lastInvoiceSub} accent="#4F8EF7"/><KPICard label={t.kpi.ytdInvoiced} value="$720,800" sub={t.kpi.ytdInvoicedSub} accent="#A78BFA"/></div><Card><SectionHeader title={t.invoices.history} sub={t.invoices.historySub} action={<span onClick={downloadAll}>{t.invoices.downloadAll}</span>}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr auto auto auto",gap:"0 24px"}}>{[t.invoices.invoiceId,t.invoices.period,t.invoices.amount,t.invoices.status,t.invoices.action].map(h=>(<div key={h} style={{fontSize:11,color:T.textLabel,fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase",padding:"0 0 14px 0",borderBottom:`1px solid ${T.tableSectionBorder}`}}>{h}</div>))}{INVOICES_DATA.map(inv=>(<div key={inv.id} style={{display:"contents"}}><div style={{padding:"16px 0",borderBottom:`1px solid ${T.tableRowBorder}`,fontSize:13,color:T.text,fontFamily:"'DM Mono',monospace",fontWeight:600}}>{inv.id}</div><div style={{padding:"16px 0",borderBottom:`1px solid ${T.tableRowBorder}`,fontSize:13,color:T.textSub}}>{inv.period}</div><div style={{padding:"16px 0",borderBottom:`1px solid ${T.tableRowBorder}`,fontSize:14,fontWeight:700,color:T.text,fontFamily:"'DM Mono',monospace"}}>{fmtFull(inv.amount)}</div><div style={{padding:"16px 0",borderBottom:`1px solid ${T.tableRowBorder}`}}><Badge status={inv.status}/></div><div style={{padding:"16px 0",borderBottom:`1px solid ${T.tableRowBorder}`}}><button onClick={()=>downloadCSV(inv)} style={{fontSize:12,color:"#4F8EF7",background:"rgba(79,142,247,0.1)",border:"1px solid rgba(79,142,247,0.2)",borderRadius:8,padding:"5px 12px",cursor:"pointer",fontWeight:500}}>{t.invoices.download}</button></div></div>))}</div></Card></div>);}
